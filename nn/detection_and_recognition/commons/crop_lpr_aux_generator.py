...
# TODO reconsider necessity to inheritance from Generator. AuxGeneratorForLPRCrop has very specific purpose that differs from other generators
class AuxGeneratorForLPRCrop(Generator):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(AuxGeneratorForLPRCrop, self).__init__(**kwargs)
    ...
    def __parse_annotation(self, element):
        ...
        plate_number = _findNode(element, 'plate').text
        ...
        return truncated, difficult, box.astype(int), label, plate_number
    ...
    def __parse_annotations(self, xml_root):
        ...
        annotations = {'labels': np.empty((len(xml_root.findall('object')),), dtype=int),
                       ...
                       'platenums': np.empty((len(xml_root.findall('object')), 1), dtype=object)
                       }
        for i, element in enumerate(xml_root.iter('object')):
            try:
                truncated, difficult, box, label, plate_number = self.__parse_annotation(element)
                ...
                annotations['platenums'][i] = plate_number
            except ValueError as e:
    ...
    def resize_image(self, image):
        ...
        if self.resize_images:
            ...
            return super(AuxGeneratorForLPRCrop, self).resize_image(image)
        else:
            return image, 1
