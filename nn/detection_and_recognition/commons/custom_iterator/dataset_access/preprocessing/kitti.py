...
class KittiGenerator(Generator):
    ...
    def load_annotations(self, image_index):
        ...
        annotations = {'labels': np.empty((len(image_data),)), 'boxes': np.empty((len(image_data), 4))}

        for idx, ann in enumerate(image_data):
            annotations['boxes'][idx, 0] = float(ann['x1'])
            annotations['boxes'][idx, 1] = float(ann['y1'])
            annotations['boxes'][idx, 2] = float(ann['x2'])
            annotations['boxes'][idx, 3] = float(ann['y2'])
