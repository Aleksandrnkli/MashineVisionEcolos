...
class OpenImagesGenerator(Generator):
    ...
    def load_annotations(self, image_index):
        ...
        annotations = {'labels': np.empty((len(labels),)), 'boxes': np.empty((len(labels), 4))}
        for idx, ann in enumerate(labels):
            ...
            annotations['boxes'][idx, 0] = x1
            annotations['boxes'][idx, 1] = y1
            annotations['boxes'][idx, 2] = x2
            annotations['boxes'][idx, 3] = y2
