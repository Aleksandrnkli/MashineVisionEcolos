...
from mtcnn.model.MTCNN import ONet
import time
import copy
import torch.nn as nn
from settings import get_const_for_onet_train

learning_mode = "CAR"
linear, cls_num = get_const_for_onet_train(learning_mode)
def weights_init(m):
...
def main():
      # background class is used in MTCNN so the total number of classes in the net's output cls_num+1
    train_path = 'E:/DataSets/LicensePlate/mtcnn/imglist_anno_24_trainval.txt'
    val_path = 'E:/DataSets/LicensePlate/mtcnn/imglist_anno_24_test.txt'
    batch_size = 64
    ...
    model = ONet(linear=linear, cls_num=cls_num, is_train=True).to(device)
        ...
        for phase in ['train', 'val']:
            ...
            for i_batch, sample_batched in enumerate(dataloaders[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    # get the mask element which >= 0, only 0 and 1 can effect the cls loss
                    mask_cls = torch.ge(gt_label, 0)
                    valid_gt_label = gt_label[mask_cls]
                    valid_pred_label = pred_label[mask_cls]

                    # calculate the box loss
                    # get the mask element which != 0, only 1 and -1 can effect the box regression loss
                    mask_offset = torch.ne(gt_label, 0)
                    ...
                    if len(valid_gt_label) != 0:
                        ...
                        # calc num of classification that match ground true
                        pred = torch.max(valid_pred_label, 1)[1]  # the result tuple of two output tensors (max, max_indices) https://pytorch.org/docs/stable/generated/torch.max.html
